...
if (!APP_ID || !INSTALLATION_ID || !PRIVATE_KEY || !WEBHOOK_SECRET) {
  console.warn('Missing one or more GitHub App env vars. Webhook/API features will remain unavailable until configured.');
}
...
async function githubRest(url, token) {
  const response = await fetch(url, {
    headers: {
      ...
      Accept: 'application/vnd.github+json',
      'X-GitHub-Api-Version': '2026-03-10',
      'User-Agent': 'Saket745-Live-Profile'
    }
  });
...
function aggregateLanguages(repos) {
  ...
  for (const repo of repos) {
    for (const [name, bytes] of Object.entries(repo.languageBytes || {})) {
  ...
  return [...totals.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, 8)
    .map(([name, bytes]) => ({ name, bytes, percent: total ? Number((bytes * 100 / total).toFixed(1)) : 0 }));
}
...
async function refreshStats(event = null) {
  ...
  try {
    ...
    const gql = await githubGraphql(query, { login: USERNAME }, token);
    const contributions = gql.user.contributionsCollection;
    ...
    state = {
      ...
      contributions: {
        ...
        repositoriesContributedTo: contributions.totalRepositoriesWithContributedCommits
      },
      repositories: repoData.sort((a, b) => (b.stars - a.stars) || new Date(b.updatedAt) - new Date(a.updatedAt)).slice(0, 12),
      languages: aggregateLanguages(repoData),
...
function summarizeEvent(eventType, body) {
  return {
    ...
    actor: body.sender?.login || body.pusher?.name || null,
    title: body.pull_request?.title || body.issue?.title || body.repository?.name || null,
    url: body.pull_request?.html_url || body.issue?.html_url || body.repository?.html_url || null,
...
wss.on('connection', ws => {
  sockets.add(ws);
  ws.send(JSON.stringify({ type: 'stats:update', data: state }));
